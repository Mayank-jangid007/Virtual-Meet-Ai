...
// }
// const defaultStatusColor = "bg-gray-300/20 text-gray-800 border-gray-800/5";
...
//             const colorClass = statusColorMap[status] || defaultStatusColor;
...
//                 <Icon
//                     className={cn(
//                         // row.original.status.toLowerCase() === 'processing' && 'animate-spin'
...
export const columns: ColumnDef<MeetingGetMany[number]>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            const status = row.original.status as keyof typeof statusIconMap;
            // ✅ 2. PROVIDE A FALLBACK IF THE ICON IS NOT FOUND
            const Icon = statusIconMap[status] || HelpCircleIcon;
            const colorClass = statusColorMap[status] || defaultStatusColor;
            ...
            return (
                <Badge
                    ...
                    className={cn(
                        'capitalize [&>svg]:size-4 text-muted-foreground',
                        colorClass // ✅ 3. USE THE FALLBACK COLOR CLASS
                    )}
                >
                    <Icon
                        className={cn(
