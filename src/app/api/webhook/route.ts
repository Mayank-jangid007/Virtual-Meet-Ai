...
//                 where: {
//                     id: channelId,
//                     status: "COMPLETED",  // MeetingStatus.COMPLETED
//                 },
//             });
...
export async function POST(req: NextRequest){
    ...
    if(eventType === 'call.session_started'){
        ...
        // ✅ FIXED: Simpler query - just check if meeting exists and isn't already processed
        ...
        if(existingMeeting.status === MeetingStatus.COMPLETED || 
           existingMeeting.status === MeetingStatus.CANCELLED) {
            console.log("⏭️ Meeting already processed, skipping");
            return NextResponse.json({ status: 'ok' });
        }
        ...
        try {
            ...
            realtimeClient.updateSession({
                instructions: existingAgent.instructions,
                voice: 'alloy',  // Optional: set voice
                turn_detection: {
                    type: 'server_vad',
                    threshold: 0.5,
