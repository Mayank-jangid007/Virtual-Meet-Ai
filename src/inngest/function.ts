...
export const meetingsProcessing = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const response = await step.run('fetch-transcript', async () => {
      ...
      return fetch(event.data.transcriptionUrl).then((res) => res.text()); //-- isko un-comment
    })

    // const response = await step.fetch(event.data.transcriptionUrl); //-- isko bhi 
...
// ek bar woo jo video me hia wo krkna old version dusra jese pehla tha wese bus 
